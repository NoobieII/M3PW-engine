...
//gcc models.c engine/pwmath.c -m32 -msse -lmingw32 -o models -static
...
int main(int argc, const char *argv[]){
	...
	for(i = 0; i < vertex_count; ++i){
		vertices_selected[i] = vertices_left[i] = &vertices[i];
	}
	...
	group_transforms[0] = PWM_ini(1.0f, NULL);
	group_level = 1;
	do{
		...
		group_transforms[group_level] = PWM_mul(PWM_inv(group_transforms[group_level - 1]), PWM_translation(origin));
		
		//retrieve vertices for part (use planes to select)
		printf("Is selection ok? (0) No, (1) Yes, (2) No/Load Selected, (3) Enter indices: ");
		scanf("%d", &selection_ok); vf_log("%d ", selection_ok);
		
		if(selection_ok == 2){
			for(i = 0; i < vertex_count; ++i){
				if(vertices_left[i] && vertices_saved[i]){
					vertices_selected[i] = vertices_saved[i];
					vertices[i].color = 0xff0000ff;
				}
				else{
		...
		while(selection_ok != 1){
			...
			printf("Is the selection ok? (0) No, (1) Yes, (2) Reset, (3) Save Selection: ");
			scanf("%d", &selection_ok); vf_log("%d ", selection_ok);
			...
			if(selection_ok == 2){
				for(i = 0; i < vertex_count; ++i){
					...
					if(vertices_selected[i]){
						vertices_selected[i]->color = 0xff0000ff;
					}
				}
			...
			if(selection_ok == 3){
				for(i = 0; i < vertex_count; ++i){
					vertices_saved[i] = vertices_selected[i];
				}
			}
		}
		...
		for(i = 0; i < 16; ++i){
			fprintf(model_out, "%s ", convert(str, group_transforms[group_level].elements[i]));
		}
